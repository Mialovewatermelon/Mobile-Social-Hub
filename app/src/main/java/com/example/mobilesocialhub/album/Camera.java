...
public class Camera extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            imageUri = FileProvider.getUriForFile(this, "com.example.mobilesocialhub.fileprovider", outputImage);
        ...
        intent3 = new Intent(getApplicationContext(), TestActivity.class);
        pictureSave.setOnClickListener(new com.example.mobilesocialhub.album.Camera.pictureSaveFunction());
...
//            System.out.println(imageUri.getPath());
            intent3.putExtra("url",pathForMain);
    ...
    public void saveToSystemGallery(Bitmap bmp) {
        // 首先保存图片
//        File appDir = new File(Environment.getExternalStorageDirectory(), "camera3");
        File appDir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM).toString());

        String fileName = System.currentTimeMillis() + ".jpg";
        System.out.println(fileName);
        File file = new File(appDir, fileName);
        pathForMain = file.getAbsolutePath();
